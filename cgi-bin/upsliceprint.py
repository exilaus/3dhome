...
if fileitem.filename:
   ...
   open( './storage/stl/' + fn, 'wb').write(fileitem.file.read())
...
#subprocess.call("./admesh --normal-directions --tolerance=0.01 --exact -a ./storage/stl/%s ./storage/stl/%s > stl-stats.txt" % (fn,fn),shell=True)
execute("./admesh --normal-directions --tolerance=0.01 --exact -a ./storage/stl/%s ./storage/stl/%s > ./stl-stats.txt" % (fn,fn))
...
if MAXX/x<1 or MAXY/y<1 or MAXZ/z<1 :
   ...
   execute("./admesh --normal-directions --tolerance=0.01 --exact --scale=%f -a ./storage/stl/%s ./storage/stl/%s > ./stl-stats.txt" % (scale,fn,fn))
